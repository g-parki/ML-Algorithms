...
#Load model, extract components
with open('poly_linear_regression.json') as f:
    model = json.load(f)

coef_x, coef_x2 = model['coefficients'][0], model['coefficients'][1]
intercept = model['intercept']
score = round(model['score'], 2)
corresponding_y = lambda x: intercept + (coef_x*x) + (coef_x2*x*x)
...
p.line(x='x', y='y', color="red", source=line_source)
...
callback = CustomJS(args={'slider_data': slider_coordinates, 'intercept': intercept, 'coef_x': coef_x, 'coef_x2': coef_x2, 'label': labels}, code= """

    const x = cb_obj.value;
    const y = intercept + (coef_x*x) + (coef_x2*x*x);
...
with open('poly_simple_linear.html', 'w') as f:
    f.writelines(html)
